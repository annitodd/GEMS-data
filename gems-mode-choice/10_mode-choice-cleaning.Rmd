...
hhct is restricted data. maps households to trips. hhpub has public info for household information demographics location of household info from survey
...
with imputation
```{r}
df_geoID <- read_csv(file=file.path(data_results, 'raw/ccst_geoid_key_transp_geo_with_imputation.csv'))
names(df_geoID)
```
...
Examine the unique primary keys to make sure they're unique. These should be 0, meaning that these uniquely identify the observations:
...
```{r}
df_temp <- full_join(df_trippub,df_tripct,
...
```{r}
df_temp2 <- full_join(df_temp,df_hhct,
                     by=c("HOUSEID"))
df_temp2
```
...
Save the complete merged file as a parquet file and a csv file

```{r}
write_parquet(df_temp2,  file.path(data_results, "10-mode-choice-cleaning_output-full-merged.parquet"))
write_rds(df_temp2,  file.path(data_results, "10-mode-choice-cleaning_output-full-merged.rds"))

write_csv(df_temp2,  file.path(data_results, "10-mode-choice-cleaning_output-full-merged.csv"))
```
