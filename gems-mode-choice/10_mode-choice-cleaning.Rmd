...
```{r}
df_geoID_origin <- df_geoID_origin %>% 
  mutate(origin_geo2types = 
      case_when(origin_geotype_ATB %in% c("A","B") ~ 'AB',
                origin_geotype_ATB %in% c("C","D","E","F","G") ~ 'CDEF',
                TRUE ~ "other")
      )
summary <- df_geoID_origin |>
    group_by(origin_geotype_ATB, origin_geo2types) |>
  summarise(countN = n() ,
            Nmissing = sum(is.na(origin_geo2types)),
    .groups = "drop") |> 
  arrange(origin_geo2types)   
summary
```
...
```{r}
df_geoID_origin <- df_geoID_origin %>% 
  unite(origin_microtypeXorigin_geo2types,
        c("origin_microtype_ATB","origin_geo2types"),
        sep="_",remove = FALSE)
```
...
```{r}
df_geoID_dest <- df_geoID_dest %>% 
  unite(dest_microtypeXdest_geo2types,
        c("dest_microtype_ATB","dest_geo2types"),
        sep="_",remove = FALSE)
```
...
```{r}
df_geoID_hh <- df_geoID_hh %>% 
  unite(hh_microtypeXhh_geo2types,
        c("hh_microtype_ATB","hh_geo2types"),
        sep="_",remove = FALSE)
```
...
Save only the variables we might want into a smaller file (Currently nothing here)
