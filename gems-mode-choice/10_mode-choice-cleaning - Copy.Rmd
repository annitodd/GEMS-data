...
![](pngs/NHTS_Schematic.png){width="480"}

![](pngs/NHTS_Summary_of_content.png){width="300"}

![](pngs/NHST_data_file_structure.png){width="480"}

-   trippub: each row is person-trip, without geo IDs. It's public dataset.
-   tripsct: County-Tract Crosswalk
-   hhct.csv: Household GEOIDs
...
it is possibly the raw trip with od  -- only has tract
...
# DATA - Merge
Examine the unique primary keys
```{r}
#these should be 0, meaning that these uniquely identify the observations:
df_trippub |>  
...
df_temp2 <- full_join(df_temp,df_hhct,
                     by=c("HOUSEID"))
df_temp2
df_temp2 <- left_join(df_temp,df_hhct,
                     by=c("HOUSEID"))
df_temp2
View(df_temp2)
```
